...
<body>
	<div class="cusalldiv">
		...
		<div class="usechange">
			<h2>${code }</h2>
			<form
				action="/ERP-NBEmail/helpServlet?action=SalesRank&className=CustomerServlet"
				method="post">
				<table class="usectable">
					<tr>
						...
						<td class="usermatd3">根据跟单销售查询:</td>
						<td>
							<div class="userselediv_nor">
								<select name="condition2" id="audit" class="userselein">
									<option value="-1">全部</option>
									<option value="1"
										<c:if test="${fyfx==0 }">selected="selected"</c:if>>LynnYuan</option>
									<option value="2"
										<c:if test="${fyfx==1 }">selected="selected"</c:if>>Fionayan</option>
									<option value="3"
										<c:if test="${fyfx==2 }">selected="selected"</c:if>>Amyzhao</option>
									<option value="4"
										<c:if test="${fyfx==3 }">selected="selected"</c:if>>PaulWang</option>
									<option value="5"
										<c:if test="${fyfx==4 }">selected="selected"</c:if>>Joannahao</option>
									<option value="6"
										<c:if test="${fyfx==5 }">selected="selected"</c:if>>ChloeMa</option>
									<option value="7"
										<c:if test="${fyfx==6 }">selected="selected"</c:if>>ElaineSheng</option>
									<option value="8"
										<c:if test="${fyfx==7 }">selected="selected"</c:if>>IvyWu</option>
									<option value="9"
										<c:if test="${fyfx==8 }">selected="selected"</c:if>>JessieHan</option>
									<option value="10"
										<c:if test="${fyfx==9 }">selected="selected"</c:if>>KathyPan</option>
									<option value="11"
										<c:if test="${fyfx==10 }">selected="selected"</c:if>>SherryZhou</option>
									<option value="12"
										<c:if test="${fyfx==11 }">selected="selected"</c:if>>JuliaZeng</option>
									<option value="13"
										<c:if test="${fyfx==12 }">selected="selected"</c:if>>anna</option>
									<option value="14"
										<c:if test="${fyfx==13 }">selected="selected"</c:if>>tina</option>
								</select>
							</div>
						</td>
				...
				<table class="usectable">
					<tr>
						<td class="usermatd4"><a class="mana_inbtn"
							...
							href="/ERP-NBEmail/helpServlet?action=Customere&className=CustomerServlet" target="_blank">最近36个月有下单
								但 最近24个月没下单的</a> <a class="mana_inbtn"
							href="/ERP-NBEmail/helpServlet?action=Customerf&className=CustomerServlet" target="_blank">最近48个月有下单
								但 最近36个月没下单的</a> <a class="mana_inbtn"
			...
			<table class="emanagergettable">
				...
				<c:forEach items="${cusList1}" var="cus" varStatus="i">
					<tr <c:if test="${i.count%2==0}">bgcolor="#cff7c9"</c:if>>
						...
						<td>${cus.amount != null ?fn:substring(cus.amount,0,fn:indexOf(cus.amount,".")):""}</td>
						<td>${cus.saleName1 }</td>
						<td>${cus.saleName2 }</td>
