...
<body>
	<div class="cusalldiv">
		...
		<div style="border: 1px solid #BBBABA; width: 100%; margin: auto;">
				...
				<!-- <a target="_blank" href="/ERP-NBEmail/helpServlet?action=invoiceDocumentarySaleOwnedToUs&className=InvoiceServlet" ><button>跟单名下欠我司发票的所有工厂</button></a>
					<a target="_blank" href="/ERP-NBEmail/helpServlet?action=invoiceDocumentaryPurchaseOwnedToUs&className=InvoiceServlet" ><button>采购名下欠我司发票的所有工厂</button></a>
					  -->
			</div>
			...
			<table class="emanagergettable">
				...
				<c:forEach items="${cusList}" var="cus" varStatus="i">
				 <tr>
						<td>${i.count }</td>
						<td>${cus.customerManager }</td>
						<td>${cus.kingdee }</td>
						<td>${cus.factoryName }</td>
						<td><a href="/ERP-NBEmail/helpServlet?action=allDetailedAccounts&className=InvoiceServlet&kingdee=${cus.kingdee }&num=0" target="_blank"><fmt:formatNumber value="${cus.allUnacceptableInvoiceAmounts}" type="number" maxFractionDigits="2"/></a></td>
						<td><a href="/ERP-NBEmail/helpServlet?action=allDetailedAccounts&className=InvoiceServlet&kingdee=${cus.kingdee }&num=1" target="_blank"><fmt:formatNumber value="${cus.amountOfNonReceiptAndReceiptOfInvoices}" type="number" maxFractionDigits="2"/></a></td>
						<td><a href="/ERP-NBEmail/helpServlet?action=allDetailedAccounts&className=InvoiceServlet&kingdee=${cus.kingdee }&num=2" target="_blank"><fmt:formatNumber value="${cus.amountOfUnpaidInvoices}" type="number" maxFractionDigits="2"/></a></td>
