...
function explain(cid){
	...
	newWindow = window.open("/ERP-NBEmail/helpServlet?action=explain&className=CustomerServlet&cid="+cid+"","windows","height=400,width=700,top=100,left=300,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
...
<body>
	<div class="cusalldiv">
		...
		<div class="usechange">
			<h2>所有客户列表</h2>
			<form
				action="/ERP-NBEmail/helpServlet?action=LookCustomer&className=CustomerServlet"
				method="post">
			...
			<table class="emanagergettable">
				...
				<c:forEach items="${cusList}" var="cus" varStatus="i">
					<tr <c:if test="${i.count%2==0}">bgcolor="#cff7c9"</c:if>>
						...
						<td>${cus.firstName }</td>
						<td>${cus.projectId }${cus.projectDesce }
							下单时间:${cus.createTime != null ?fn:substring(cus.createTime,0,fn:indexOf(cus.createTime," ")):""}日
						...
						<td>${cus.cstatus}${cus.work} ${cus.kehudaxiao} ${cus.note}
						</td>
